...
int main() {
    ...
    auto menu = candidateMenuPosition(candidate, 160, 110, 120, inputAbove);
    FCITX_ASSERT(menu.top() == candidate.bottom());

    // When KWin flips the input popup above the caret, use that same side.
    const Rect inputBelow(40, 50, 56, 70);
    menu = candidateMenuPosition(candidate, 160, 110, 120, inputBelow);
    FCITX_ASSERT(menu.bottom() == candidate.top());
    ...
    menu = candidateMenuPosition(candidate, 160, 220, 80, inputRight);
