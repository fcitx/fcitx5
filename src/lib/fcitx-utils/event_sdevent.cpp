...
{
    uint32_t result = 0;
    if (flags & IOEventFlag::In) { result |=  EPOLLIN; }
    if (flags & IOEventFlag::Out) { result |=  EPOLLOUT; }
    if (flags & IOEventFlag::Err) { result |=  EPOLLERR; }
    if (flags & IOEventFlag::Hup) { result |=  EPOLLHUP; }
    if (flags & IOEventFlag::EdgeTrigger) { result |=  EPOLLET; }
    return result;
}
...
EventSourceIO* EventLoop::addIOEvent(int fd, IOEventFlags flags, IOCallback callback)
{
    ...
    int err;
    if ((err = sd_event_add_io(d->event, &sdEventSource, fd, IOEventFlagsToEpollFlags(flags), IOEventCallback, source.get())) < 0) {
        throw EventLoopException(err);
    }
...
EventSourceTime* EventLoop::addTimeEvent(clockid_t clock, uint64_t usec, uint64_t accuracy, TimeCallback callback) {
    ...
    sd_event_source *sdEventSource;
    int err;
    if ((err = sd_event_add_time(d->event, &sdEventSource, clock, usec, accuracy, TimeEventCallback, source.get())) < 0) {
        throw EventLoopException(err);
    }
    source->setEventSource(sdEventSource);
    return source.release();
