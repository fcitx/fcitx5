...
template<typename... Types> class Chainer;
...
public:
    ...
    void scanFiles(Type type, const std::string &path, std::function<bool(const std::string & path, const std::string &dir, bool user)> scanner);
    ...
    std::map<std::string, StandardPathFile> openMultipleFilesFilter(Type type, const std::string &path, int flags, std::function<bool(const std::string & path, const std::string &dir, bool user)> filter);
    template<typename... Args>
    std::map<std::string, StandardPathFile> openMultipleFiles(Type type, const std::string &path, int flags, Args... args) {
        return openMultipleFilesFilter(type, path, flags, filter::Chainer<Args...>(args...));
    }
