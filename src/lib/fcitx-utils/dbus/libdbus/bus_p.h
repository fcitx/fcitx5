...
class DBusObjectVTableSlot : public DBusObjectSlot {
public:
    DBusObjectVTableSlot(const std::string &path, const std::string &interface,
                         ObjectVTableBase *obj,
                         ObjectVTableBasePrivate *objPriv)
        : DBusObjectSlot(path, [this](Message msg) { return callback(msg); }),
          interface_(interface), obj_(obj), objPriv_(objPriv), xml_(getXml()) {}

    ~DBusObjectVTableSlot() {}

    std::string getXml();

    bool callback(Message message);
