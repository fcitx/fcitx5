...
#include "standardpath_p.h"

namespace fcitx {

namespace {

std::string_view removeUncOrLongPathPrefix(std::string_view path) {
...
std::filesystem::path userPath(StandardPathsType type) {
    ...
    path = stringutils::replaceAll(std::string(removeUncOrLongPathPrefix(path)),
                                   "\\", "/");
...
result = SHGetKnownFolderPath(id);
}

} // namespace

StandardPathPrivate::StandardPathPrivate(
    ...
    configDirs_ =
        defaultPaths("XDG_CONFIG_DIRS", "/etc/xdg", builtInPathMap, nullptr);
