...
const struct wl_keyboard_listener WlKeyboard::listener = {
    [](void *data, wl_keyboard *wldata, uint32_t format, int32_t fd, uint32_t size) {
    ...
    [](void *data, wl_keyboard *wldata, uint32_t serial, wl_surface *surface, wl_array *keys) {
        auto obj = static_cast<WlKeyboard *>(data);
        assert(*obj == wldata);
        {
    ...
    [](void *data, wl_keyboard *wldata, uint32_t serial, uint32_t time, uint32_t key, uint32_t state) {
        auto obj = static_cast<WlKeyboard *>(data);
        assert(*obj == wldata);
        {
    ...
    [](void *data, wl_keyboard *wldata, uint32_t serial, uint32_t modsDepressed, uint32_t modsLatched, uint32_t modsLocked, uint32_t group) {
        auto obj = static_cast<WlKeyboard *>(data);
        assert(*obj == wldata);
        {
            return obj->modifiers()(serial, modsDepressed, modsLatched, modsLocked, group);
        }
    },
...
WlKeyboard::WlKeyboard(wl_keyboard *data) : version_(wl_keyboard_get_version(data)), data_(data, &WlKeyboard::destructor) {
