...
public:
    ...
    static constexpr const wl_interface *const wlInterface = &zwp_input_method_context_v1_interface;
    ...
    ZwpInputMethodContextV1(ZwpInputMethodContextV1 &&other) : data_(std::move(other.data_)) {}
    ...
    void modifiers(uint32_t serial, uint32_t modsDepressed, uint32_t modsLatched, uint32_t modsLocked, uint32_t group);
...
private:
    ...
    fcitx::Signal<void(const char *, uint32_t, uint32_t)> surroundingTextSignal_;
