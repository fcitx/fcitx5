...
XCBConnection::XCBConnection(XCBModule *xcb, const std::string &name)
    ...
    ioEvent_.reset(eventLoop.addIOEvent(fd, IOEventFlag::In, [this](EventSource *, int, IOEventFlags) {
        onIOEvent();
        return true;
    }));
    ...
    if (response_type == XCB_CLIENT_MESSAGE) {
        ...
        if (xkbEvent->any.deviceID == coreDeviceId_) {
            ...
            case XCB_XKB_STATE_NOTIFY: {
                ...
                xkb_state_update_mask(state_.get(), state->baseMods, state->latchedMods, state->lockedMods,
...
void XCBConnection::updateKeymap() {
    ...
    if (hasXKB_) {
        keymap_.reset(
            xkb_x11_keymap_new_from_device(context_.get(), conn_.get(), coreDeviceId_, XKB_KEYMAP_COMPILE_NO_FLAGS));
        if (keymap_) {
            new_state = xkb_x11_state_new_from_device(keymap_.get(), conn_.get(), coreDeviceId_);
        }
    }
    ...
    if (!keymap_) {
        ...
        if (!names[0].empty()) {
            ...
            keymap_.reset(xkb_keymap_new_from_names(context_.get(), &xkbNames, XKB_KEYMAP_COMPILE_NO_FLAGS));
        }
...
XkbRulesNames XCBConnection::xkbRulesNames() {
    ...
    xcb_get_property_cookie_t get_prop_cookie =
        xcb_get_property(conn_.get(), false, root_, xkbRulesNamesAtom_, XCB_ATOM_STRING, 0, 1024);
    auto reply = makeXCBReply(xcb_get_property_reply(conn_.get(), get_prop_cookie, NULL));
...
XCBModule::XCBModule(Instance *instance) : instance_(instance) { openConnection(""); }
...
void XCBModule::openConnection(const std::string &name_) {
    ...
    try {
        auto iter =
            conns_.emplace(std::piecewise_construct, std::forward_as_tuple(name), std::forward_as_tuple(this, name));
