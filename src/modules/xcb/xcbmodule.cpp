...
void XCBConnection::onIOEvent() {
    ...
    while (true) {
        auto event = makeXCBReply(xcb_poll_for_event(m_conn.get()));
        for (auto &callback : m_filters) {
            if (callback(m_conn.get(), event.get())) {
                break;
            }
        }
    }
...
void XCBConnection::updateKeymap() {
    ...
    if (!m_keymap) {
        ...
        int length;
        char *nameString = xkbRulesNames(&length);
        if (nameString) {
...
char *XCBConnection::xkbRulesNames(int *pLength) {
    ...
    void *data = xcb_get_property_value(reply.get());
...
XCBModule::XCBModule(Instance *instance) : m_instance(instance) {}
