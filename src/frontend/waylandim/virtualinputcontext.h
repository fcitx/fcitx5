...
public:
    ...
    bool realFocus() const { return realFocus_; }
...
class VirtualInputContextManager {
    ...
    using AppMonitorType = AppMonitor<AppKey>;
...
private:
    void appUpdated(const std::unordered_map<AppKey, std::string> &appState,
    ...
    AppMonitorType *app_;
    std::unordered_map<AppKey, std::string> lastAppState_;
    std::unordered_map<AppKey, std::unique_ptr<InputContext>> managed_;
    std::optional<AppKey> focus_;
};
